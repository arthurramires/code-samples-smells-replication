...
def prepareGraph(
    ...
    percentageHighCentralityAuthors = (
        numberHighCentralityAuthors / len(allRelatedAuthors)
        if len(allRelatedAuthors) > 0 else 0
    )
    ...
    # calculate TFC
    # FIX: prevent division by zero
    total_items = sum(authorItems.values())
    tfc = (sum(authorItems[author] for author in highCentralityAuthors) / total_items * 100) if total_items > 0 else 0
